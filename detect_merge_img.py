...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    merge_img = cv2.imread(str(source)+'.tif')  
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        ...
                        xywh_m = (xyxy2xywh(torch.tensor(xyxy_merge).view(1, 4)) / gn_m).view(-1).tolist()
                        line_m = (cls, *xywh_m, conf) if save_conf else (cls, *xywh_m)  # label format
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy_merge, merge_img, label=label, color=colors(c, True), line_thickness=line_thickness)
                        if save_crop:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/mnt/e/github/object-detection-zoo/runs/train/yolov5m5/weights/best.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--project', default='runs/detect/201005069/', help='save results to project/name')
    parser.add_argument('--name', default='yolov5m_TTA', help='save results to project/name')
